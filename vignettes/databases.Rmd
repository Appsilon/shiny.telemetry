...
The command below will create a data storage instance with the necessary configuration for each database backend. 
The arguments vary from each database backend, however, the operations are standard across all of them.

### PostgreSQL

The PostgreSQL data storage will create a database in a Postgres server.
